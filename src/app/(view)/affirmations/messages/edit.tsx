...
export default function Edit({
  ...
  id: string;
  dataset: { text: string; category_id: string | undefined };
}) {
  const [{ token }] = useCookies(["token"]);
  const [selectedCat, setSelectedCat] = useState<string | undefined>(
    dataset.category_id
  );
  const [text, setText] = useState(dataset.text);
  ...
  const { mutate } = useMutation({
    mutationKey: ["add_aff"],
    mutationFn: () => {
      return updateAffirmation(id, text, token);
    },
    onError: (err) => {
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Update Affirmation Message</DialogTitle>
        </DialogHeader>
